...
/// One tree node for a peeked folder: header carries the recursive rollup so
/// a folder can be judged without expanding it.
public sealed class PeekFolderItem
{
    public PeekFolderItem(PeekFolder folder)
    {
        Header = folder.Unreadable ? $"{folder.Name} · unreadable" : $"{folder.Name} · {folder.Rollup}";
        IsUnreadable = folder.Unreadable;
        Children =
        [
            .. folder.Folders.Select(f => (object)new PeekFolderItem(f)),
            .. folder.Files.Select(f => (object)new PeekFileItem(f)),
        ];
    }
...
public sealed class PeekFileItem(PeekFile file)
{
...
/// The Folder Peek window's state: one folder at a time, listing only, never
/// decoding. View-only by identity; nothing here can touch the files.
public sealed class FolderPeekViewModel(AppSettings settings) : ObservableObject
{
    public ObservableCollection<PeekFolderItem> RootItems { get; } = [];

    private string? _folder = settings.FolderPeekFolder;
    public string? Folder { get => _folder; private set => Set(ref _folder, value); }

    private bool _isLoading;
    public bool IsLoading { get => _isLoading; private set => Set(ref _isLoading, value); }

    private string _footerText = "Pick a folder to peek inside.";
    public string FooterText { get => _footerText; private set => Set(ref _footerText, value); }

    public PeekFolder? LastRoot { get; private set; }
    ...
    public async Task LoadAsync(string folder)
    {
        ...
        try
        {
            ...
            PeekFolder root;
            try { root = await Task.Run(() => FolderPeek.Build(folder, localCache)); }
        ...
        {
            SetError($"Peek failed: {ex.Message}");
        }
        finally
        {
            IsLoading = false;
    ...
    private static (int Files, int Folders) CountOf(PeekFolder f)
    {
