...
public sealed record Fingerprint(byte FramesPerSecond, uint[] Words)
{
    ...
    public static Fingerprint? FromBlob(byte[] blob)
    {
        ...
        // (long) so a hostile count near int.MaxValue / 4 cannot overflow the
        // multiply and slip a huge allocation past the length check.
        if (count < 0 || blob.Length != 6L + (long)count * 4) return null;
