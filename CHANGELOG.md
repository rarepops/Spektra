...
### Added
- A `Difference` entry in the channel list of any file with two or more channels, beside `Mix`. Mix shows what the file sounds like; Difference shows only what its left and right channels do not share, which answers three questions the mix cannot. A stereo file carrying mono content shows nothing at all. A lossy source dressed up as lossless often gives itself away here first, because joint-stereo encoders spend the fewest bits on exactly this part of the signal, so it stops lower than the mix does. And a file with one channel wired backwards, which cancels its vocals and bass out of the mix and looks broken, shows its whole track. The loudness check and the two-file comparison tools decode whichever channel is selected, so they see the difference too when it is picked. It is a view rather than an audit column on purpose: most stereo files carrying mono content are recordings that were simply made in mono, so flagging them across a library would be noise.
- Right-clicking a file offers `Play in default player`, in the audit grid and folder tree, in the Folder Manifest, and on both halves of Duplicate Detective. Spektra draws audio and never plays it, so choosing between two copies meant leaving the window, finding the file again in Explorer and opening it from there, at exactly the moment the spectrogram has said the two differ and only listening will say which one to keep. The file goes to whatever the system already opens it with: the file association on Windows, `open` on macOS, `xdg-open` on Linux. In the Folder Manifest, which lists every file in a folder rather than only the audio, the verb is greyed out for everything that is not audio, so nothing else can be launched from it. If nothing is registered to play the file, or the file has been deleted since the scan that listed it, the window says so on its own status line rather than doing nothing.

### Changed
- A folder diff tells "the other folder has not got this" apart from "the other folder has it under a name that says so". The columns used to hold every file that matched nothing by audio, and audio is a strict judge: on the owner's library 278 of the 319 files in those columns were sitting on the other side all along, under the same name or under one crediting a different set of featured artists, leaving 41 the other folder genuinely lacked. Finding those 41 meant reading past 278 that were never missing. A file with a counterpart is now labelled with the name it goes by over there, sorted below the files without one, counted separately in its column's summary and in the footer, and hidden by default behind a new `Hide name matches` box beside `Only differences`. Grouping is untouched and stays what it has always been, audio and nothing else, because a name is not evidence worth deleting a file over; a diff asks a different question, where a wrong "you already have it" costs a second look and a wrong "you are missing it" costs a hunt through hundreds of files, so it may use evidence the grouper may not. Names are compared as sets of words with the release-format noise dropped, so `Blood Stain Child` finds `BLOOD STAIN CHILD` and `Red Dawn.opus` finds `Red Dawn.flac`. Two names match when their words are the same, or when one name's words sit wholly inside the other's and at least three of them do, which is what separates `Thomas Bergersen; Two Steps from Hell - Heart of Courage` from `Thomas Bergersen - Heart of Courage` (the same track) while keeping `Alexei Scutari - Dacia` and `Alexei Scutari - Spirit` apart (two tracks that merely share an artist).
- Clear all in Duplicate Detective clears the results too, not just the folder list. It used to leave the last scan on screen, footer included, still counting groups found in folders that were no longer listed anywhere; nothing about that display looks stale, so it reads as current. Removing a single folder still keeps the results, because that is usually recomposing a set they still describe, while clearing everything means starting over. Clearing after emptying the list one folder at a time now works as well, which it did not: the button gave up early whenever the list was already empty, results or no results.
...
### Fixed
- The `possibly the same · check these yourself` list in a folder diff can actually be checked. Each entry was a line of text that named a group and gave no way into it, so the files it asked you to check could not be opened, played, compared or even copied from there, and the only way to reach them was to leave the diff for the ordinary view and find the group again. Each entry is now the same foldout the ordinary view uses, closed by default and only as tall as its text, so the list still reads as a list of names, with every file inside carrying the full right-click menu, and the entry itself offering `Copy name` and `Copy all paths`. It deliberately leaves out `Copy loser paths`: these are the matches Spektra is unsure of, and that verb exists to feed a delete. The list is alphabetical now, like the two columns above it, where it used to follow how much space each group would reclaim, which is the duplicate hunt's question and meant nothing in a diff. Its height is dragged at its top edge and remembered, where it used to be fixed at a size that would have fitted about four of the new entries. `Copy name` is on the ordinary view's groups as well.
- Duplicate Detective finds duplicates it used to walk straight past, including files whose fingerprints are identical. Matching votes on how far apart two files start and scores them at that alignment, and it counted the votes for each candidate alignment together with its two neighbours, so that a real alignment falling between two frames is not split in half. That sum then decided the alignment as well as ranking it, and a window centred one frame off a peak still contains that peak, so it could out-total the window centred on it. Music makes that the normal case rather than the exception: a word recurs two frames back far more often than one frame back. Worse, two copies of one track have a perfectly symmetric vote histogram, so both neighbours tie and one wins outright. Every such pair was then scored one frame out of step, which agrees no better than two unrelated tracks do. On the owner's library this lost 146 pairs of byte-identical fingerprints, each scored zero, and re-matching that library recovers 96 groups and 224 files, leaving 316 unmatched files where there were 540. The alignment is now the best single offset inside the winning window, and the largest group is unchanged at six members, so nothing new is being chained together.
- A folder diff's columns can be told apart by their headings. Each column was headed with its folder's last path segment, which is the one part two copies of a library are guaranteed to share: comparing `D:\FLAC\Album` with `D:\MP3\Album` produced two columns both headed `Album`, distinguishable only by hovering for the full path, in the view whose entire job is telling two folders apart. Each heading is now the shortest ending no other scanned folder shares, so those two read `FLAC\Album` and `MP3\Album`, only the headings that actually collide grow, and folders differing solely by drive fall back to their whole paths, because a drive letter is not a path segment and no amount of growing would ever reach it.
- Weak matches no longer crowd a folder diff off the screen. The `possibly the same · check these yourself` panel grew to fit however many it had, and it sits in the row that the columns hand their leftover height to, so a scan with a long weak-match list squeezed the columns, which are the reason the view was opened, down to a sliver. It now scrolls inside the same height the folder list at the top of the window uses, with the count above the scroll so it stays readable.

## [0.24.4] - 2026-09-04

### Fixed
- Installing from npm works on Linux. It did not in 0.24.1 or 0.24.3, because the package holding the Linux binary is one npm accepts publishes to and then never serves: both versions were taken, both reported success, and the registry returns 404 for the package and its tarball while refusing a republish because versions are immutable. Nothing on this side can repair that, and 0.24.3 proved a new version does not route around it, so the Linux binary now ships as `@rarepops/spektra-cli-linux-x64-glibc`, which is a fresh package and honest anyway, since the build needs glibc and refuses to run on musl. Windows and macOS were never affected and are unchanged. `npm install -g spektra-cli` is still the whole command.
- A release can no longer publish a tool that cannot be installed. `npm publish` exits 0 and prints `+ name@version` for a package the registry then 404s, which is how 0.24.3 announced "5 published" with one package missing and shipped a dispatcher naming a dependency nobody could fetch. Every package is now read back from the registry before the next one is published, and the dispatcher goes last, so a swallowed publish stops the release instead of completing it. Slowness is not failure: a package has taken over four minutes to appear after a successful publish, so the read-back waits about ten and says how long it has been waiting.
- The post-publish smoke test no longer reports a slow package as a broken one. It retried the dispatcher 404ing, but a platform package that is not being served yet produces no error at all: the dependency is optional, npm skips it in silence, and the install succeeds. It now retries the check that matters rather than the install, which is what turned a macOS package that was two minutes late into a failed release.
...
### Fixed
- npm installation works on Windows and macOS, with a provenance attestation on every package. Linux is still missing here, for the same reason it was missing in 0.24.1: the package holding the Linux binary is one npm takes publishes for and never serves. That was assumed at the time to be fallout from a registry incident on 3 September, and publishing 0.24.3 into it disproved that, so it is fixed in 0.24.4 rather than here. Every other way of getting the tool, Linux included, is unaffected.
...
- A crash log. When something unexpected ends Spektra, what happened is now written to `crash.log` under `%LOCALAPPDATA%\Spektra` before the process goes: when it happened, which build, and the full error with its stack. Until now a crash left nothing behind at all, so a bug report could only say "it disappeared"; attaching this file says the rest. The log is capped and keeps one previous generation beside it, so it never grows without bound, and background errors that do not end the app are recorded in the same place, since a bug that fails silently is still a bug.
...
- A stray keypress can no longer fail a file. The streaming decode left ffmpeg listening to the terminal for its own interactive commands, unlike every other pass, so typing into the console mid-audit (a `q` above all) could abort the decode in flight, and the file was then reported as failing to decode. All passes now tell ffmpeg to ignore the keyboard.
- One misbehaving program can no longer break "opening a file brings up the window you already have". Later launches hand their command line to the running instance over a pipe, and the listener waited forever for whoever connected to say something: any process that connected and went silent parked it for good, after which every launch quietly opened another window instead. A connected client now gets five seconds, generous next to the instant a real handoff takes, and then only that connection is dropped. Verified by holding the pipe open silently and launching anyway: the launch handed off the moment the timeout freed the listener, and the next one was instant.
...
[Unreleased]: https://github.com/rarepops/Spektra/compare/v0.24.4...HEAD
[0.24.4]: https://github.com/rarepops/Spektra/releases/tag/v0.24.4
