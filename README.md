...
- Duplicate Destroyer: find duplicate songs across folders by acoustic fingerprint (renames and format changes cannot hide a copy), grouped with confidence levels and the best copy starred; view-only, with HTML/CSV/JSON export
- Folder Manifest: instantly list everything inside a folder as a tree with honest type chips (cached audio verdicts included) and per-folder composition rollups; view-only, with HTML/CSV/JSON export
